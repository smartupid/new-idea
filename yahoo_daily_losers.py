...
def fetch_all_losers(count_per_page=100, pause=0.8, max_pages=200):
    all_rows = []
    start = 0
    with requests.Session() as sess:
        sess.get("https://finance.yahoo.com/markets/stocks/losers/", headers=HDRS, timeout=15)
...
if __name__ == "__main__":
    df_long = fetch_all_losers()
    print(f"Retrieved {len(df_long)} rows from API")

    # Save full dataset to yahoo_losers_long.db
    save_to_sqlite(df_long, "yahoo_losers_long.db", "losers_history")
    ...
    # Save short dataset to yahoo_losers_short.db
    save_to_sqlite(df_short, "yahoo_losers_short.db", "losers_history")
