...
def fetch_page(session, start, scr_id, count=100, timeout=15):
    params = {
        "count": count,
        "formatted": "false",
        "lang": "en-US",
...
def fetch_all_gainers(count_per_page=100, pause=0.8, max_pages=200):
    ...
    with requests.Session() as sess:
        ...
        for _ in range(max_pages):
            data = fetch_page(sess, start, "day_gainers", count_per_page)
            try:
...
def fetch_52week_gainers(max_stocks=500, count_per_page=100, pause=0.8):
    all_rows = []
    start = 0
    with requests.Session() as sess:
        sess.get("https://finance.yahoo.com/markets/stocks/52-week-gainers/", headers=HDRS, timeout=15)
        while len(all_rows) < max_stocks:
            data = fetch_page(sess, start, "52w_gainers", count_per_page)
            try:
                quotes = data["finance"]["result"][0]["quotes"]
            except (KeyError, IndexError, TypeError):
                break
...
if __name__ == "__main__":
    ...
    print(f"Retrieved {len(df_long)} daily gainers from API")
    ...
    # Fetch 52-week gainers
    df_52w = fetch_52week_gainers(max_stocks=500)
    print(f"Retrieved {len(df_52w)} 52-week gainers from API")
    save_to_sqlite(df_52w, "yahoo_52week_gainers.db", "gainers_52week_history")